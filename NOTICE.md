...
| cube-sat-eps | multi-channel, `$Component$ChannelAlpha`; harness to a fractional port end | Non-default channel designator format, and ships no `.PrjPcbStructure`, so channels must be recovered from the sheet symbols. Its `UART` harness crosses from `main_connector` to `uC` through an auto-sized port (`Width=44 \| Width_Frac=35626`) whose far end meets the harness line at `X1=314 \| X1_Frac=35626`, so the join depends on reading `_Frac` as hundred-thousandths. Ships `pcb/EPS.PcbDoc`; its board and the project's schematic PDF both put `J600.43` on `RX` with `U200.30`. |
