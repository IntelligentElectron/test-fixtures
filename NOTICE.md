...
| fmc-dio-32chlvdsa | [ohwr/project/fmc-dio-32chlvdsa](https://gitlab.com/ohwr/project/fmc-dio-32chlvdsa) | CERN-OHL-1.1 | CERN |
| easyinverter | [hildogjr/easyinverter](https://github.com/hildogjr/easyinverter) | GPL-3.0 | Hildo Guillardi Júnior and contributors |
...
| cube-sat-eps | multi-channel, `$Component$ChannelAlpha`; harness to a fractional port end | Non-default channel designator format, and ships no `.PrjPcbStructure`, so channels must be recovered from the sheet symbols. Its `UART` harness meets an auto-sized port (`Width_Frac`) at the port's far end. Ships `pcb/EPS.PcbDoc`. |
...
| fmc-dio-32chlvdsa | multi-channel, `Repeat()` fed by buses of another name | One buffer sheet placed 32 times by `Repeat(U_fmc_digital_io_buffers,1,32)`, its `Repeat(FMC_P)` entries fed from buses the top sheet labels `FMC1_P[32..1]`, whose member wires never touch the bus. Channel designators run past `Z` (`IC49[` ... `IC49\``). Ships its board, which agrees with the schematics on every pin. Schematics and `PCB-Layout/FMC_DIO_32ch_lvds_a.PcbDoc` only; outputs and docs pruned. |
| easyinverter | nested harnesses and nested multi-channel sheets | The `prj v2 [Luis]/Main board` project. One sheet is placed by two plain symbols, and places a sheet of its own, so the grandchild exists twice. Harness connectors hand bundles to connectors nested in their entries, and a top-level entry `DC_LINK` meets the child's port `DC_link`. Ships `MainBoard_PCB.PcbDoc`, which agrees with the schematics on every pin and every net name. |
